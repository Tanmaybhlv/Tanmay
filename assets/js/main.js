(() => {
  ...
  function initHeroAnimation() {
    ...
    tl.to(heroImg, { filter: 'grayscale(100%) brightness(0.55)', ease: 'none', duration: 0.3 }, 0.2);
  ...
  // ==========================================
  // CERTIFICATIONS — Pinned horizontal scroll
  // ==========================================
  function initFannedCards() {
    const section = document.getElementById('certsHorizontal');
    const track = document.getElementById('certsFan');
    const pinWrap = document.querySelector('.certs-pin-wrap');
    if (!section || !track || !pinWrap) return;

    const getScrollDist = () => track.scrollWidth - window.innerWidth + window.innerWidth * 0.1;
    ...
    ['.edu-card', '.pub-card', '.leadership-card', '.skill-category'].forEach(selector => {
      ...
      ScrollTrigger.batch(selector, {
        ...
        onEnter: (batch) => {
          gsap.to(batch, {
            y: 0, opacity: 1,
            stagger: 0.08, duration: 0.6, ease: EASE
          });
        }
      });
    });
